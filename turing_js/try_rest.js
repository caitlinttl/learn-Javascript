/*  
使用NodeJS 創建以下三個API：
GET：/json_file?name=xxx 獲取特定命名JSON 文件的內容
POST: /json_file?name=xxx 通過POST請求在body 中傳入JSON 數據，更新特定文件，如文件不存在，則新建文件
DELETE：/json_file?name=xxxx 刪除特定命名的JSON 文件
*/
...
app.delete('/json_file', function(req, res) {
    try {
        ...
        console.log(err);
        res.send({'error': 'Delete file failes'})
    }
});
